...
opt.epochs = 20         -- **approximate** number of passes through the training data (see below for the `iterations` variable, which is calculated from this)
...
elseif opt.optimization == 'sgd' then
  optimState = {
    learningRate = 5e-4, -- every time when we find the gradient of the parameter, how far do we move on that direction
    weightDecay = 0,
    momentum = 0,
...
-- nn.ClassNLLCriterion() is used for n class classifier
-- Inputs of the criterion are the negative log likelihood for each class.
-- Outputs are the true class of the sample.
-- The loss is the predicted log-likelihood of the true class, so the higher 
-- the probability, the closer the log likelihood is to 0.
-- To get the NLL for each class, we usually put a LogSoftMax layer after the
-- the model.
...
local feval = function(x)
  ...
  if end_index == n_train_data then
    -- we just finished a epoch
    -- now we evaluate the model performance
    local total_outputs = model:forward(train.data)
    local total_loss = criterion:forward(total_outputs, train.labels)
    total_losses[#total_losses + 1] = total_loss
    print(string.format("total loss is: %6.6f", total_loss))
    counter = 0
  else
    counter = counter + 1
  end
...
total_losses = {}    -- training losses for each epoch/batch
...
gnuplot.plot({
  torch.range(1, #total_losses),        -- x-coordinates for data to plot, creates a tensor holding {1,2,3,...,#losses}
  torch.Tensor(total_losses),           -- y-coordinates (the training losses)
  '-'})
